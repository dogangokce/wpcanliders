...
class Canliders1_Admin {
	...
	public function enqueue_styles() {
		 ...
		 * defined in Canliders1_Loader as all of the hooks are defined
		 * in that particular class.
		 *
		 * The Canliders1_Loader will then create the relationship
		 * between the defined hooks and the functions defined in this
		 * class.
		 */

		wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/canliders1-admin.css', array(), $this->version, 'all' );
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/canliders1-admin.js', array( 'jquery' ), $this->version, false );
	...
	public function create_admin_menu() {
		add_menu_page(
			__( 'Rooms', 'canliders1' ), __( 'Rooms', 'canliders1' ), 'activate_plugins', 'bbb_room',
			'', 'dashicons-video-alt2'
		);

		if ( current_user_can( 'manage_categories' ) ) {
			add_submenu_page(
				'bbb_room', __( 'Rooms', 'canliders1' ), __( 'Categories' ), 'activate_plugins',
				'edit-tags.php?taxonomy=bbb-room-category', ''
			);
		}

		add_submenu_page(
			'bbb_room', __( 'Rooms', 'canliders1' ), __( 'Settings' ), 'activate_plugins',
	...
	public function add_custom_room_column_to_list( $columns ) {
		$custom_columns = array(
			...
			'token'          => __( 'Token', 'canliders1' ),
			'moderator-code' => __( 'Moderator Code', 'canliders1' ),
			'viewer-code'    => __( 'Viewer Code', 'canliders1' ),
		);
	...
	public function display_room_server_settings() {
		...
		require_once 'partials/canliders1-settings-display.php';
	}
	...
	public function fetch_room_server_settings() {
		$settings = array(
			'bbb_url'          => get_option( 'canliders1_url', 'http://test-install.dogangokce.com/canliders1/' ),
			'bbb_salt'         => get_option( 'canliders1_salt', '8cd8ef52e8e101574e400365b55e11a6' ),
			'bbb_default_url'  => 'http://test-install.dogangokce.com/canliders1/',
	 ...
	 */
	public function canliders1_show_upgrade_notification( $current_plugin_metadata, $new_plugin_metadata = null ) {
		if ( ! $new_plugin_metadata ) {
			$new_plugin_metadata = $this->canliders1_update_metadata( $current_plugin_metadata['slug'] );
		}
	 ...
	 */
	private function canliders1_update_metadata( $plugin_slug ) {
	 ...
	 *                          1 - success
	 *                          2 - bad url format
	 *                          3 - bad canliders1 settings configuration
	 */
	private function room_server_settings_change() {
		if ( ! empty( $_POST['action'] ) && 'bbb_general_settings' == $_POST['action'] && wp_verify_nonce( sanitize_text_field( $_POST['bbb_edit_server_settings_meta_nonce'] ), 'bbb_edit_server_settings_meta_nonce' ) ) {
			...
			if ( ! Canliders1_Api::test_canliders1_server( $bbb_url, $bbb_salt ) ) {
				return 3;
			}

			if ( substr_compare( $bbb_url, 'canliders1/', strlen( $bbb_url ) - 14 ) !== 0 ) {
				return 2;
			}

			update_option( 'canliders1_url', $bbb_url );
			update_option( 'canliders1_salt', $bbb_salt );
	...
	public function check_for_heartbeat_script() {
		...
		if ( ! wp_script_is( 'heartbeat', 'registered' ) && ! get_option( 'dismissed-' . $bbb_warning_type, false ) ) {
			$bbb_admin_warning_message = __( 'CanliDers1 works best with the heartbeat API enabled. Please enable it.', 'canliders1' );
			$bbb_admin_notice_nonce    = wp_create_nonce( $bbb_warning_type );
			require 'partials/canliders1-warning-admin-notice-display.php';
		}
	}
