<?php
/**
 * Handle the majority of Canliders1 API calls to admin.
 *
...
class Canliders1_Admin_Api {
	...
	public function save_room( $post_id ) {
		...
		if ( $this->can_save_room() ) {
			...
			if ( '' == $moderator_code ) {
				$moderator_code = Canliders1_Admin_Helper::generate_random_code();
			}
			if ( '' == $viewer_code ) {
				$viewer_code = Canliders1_Admin_Helper::generate_random_code();
			}

			// Ensure the moderator code is not the same as the viewer code.
			if ( $moderator_code === $viewer_code ) {
				$viewer_code = $moderator_code . Canliders1_Admin_Helper::generate_random_code( 1 );
			}
			...
			if ( ! get_post_meta( $post_id, 'bbb-room-meeting-id', true ) ) {
				update_post_meta( $post_id, 'bbb-room-meeting-id', sha1( home_url() . Canliders1_Admin_Helper::generate_random_code( 12 ) ) );
			}
