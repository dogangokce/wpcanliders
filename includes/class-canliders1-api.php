<?php
/**
 * Static API calls to the Canliders1 Server.
 *
 ...
 * Static API calls to the Canliders1 Server.
 *
 * This class defines all code necessary to interact with the remote Canliders1 server.
 *
 * @since      3.0.0
 * @package    Canliders1
 * @subpackage Canliders1/includes
...
class Canliders1_Api {
	 ...
	 * 
	 * @return  Integer $return_code|404    HTML response of the canliders1 server.
	 */
	...
	public static function get_join_meeting_url( $room_id, $username, $password, $logout_url = null) {
		...
		if ( ! self::is_meeting_running( $rid ) ) {
			...
			if ( 200 !== $code ) {
				wp_die( esc_html__( 'It is currently not possible to create rooms on the server. Please contact support for help.', 'canliders1' ) );
			}
		}
	...
	/**
	 * Verify that the endpoint is a CanliDers1 server, the salt is correct, and the server is running.
	 *
	 * @since 3.0.0
	 *
	 * @param String $url         CanliDers1 URL endpoint to be tested.
	 * @param String $salt        CanliDers1 server salt to be tested.
	 *
	 * @return Boolean true|false Whether the CanliDers1 server settings are correctly configured or not.
	 */
	public static function test_canliders1_server( $url, $salt ) {
	...
	/**
	 * Returns the complete url for the canliders1 server request.
	 *
	 * @since   3.0.0
	 *
	 * @param   String $request_type   Type of request to the canliders1 server.
	...
	private static function build_url( $request_type, $args ) {
		...
		$url_val  = strval( get_option( 'canliders1_url', 'http://test-install.dogangokce.com/bigbluebutton/' ) );
		$salt_val = strval( get_option( 'canliders1_salt', '8cd8ef52e8e101574e400365b55e11a6' ) );
