...
class Canliders1 {
	 ...
	 * @since    3.0.0
	 * @access   protected
	 * @var      Canliders1_Loader    $loader    Maintains and registers all hooks for the plugin.
	 */
	protected $loader;
	...
	public function __construct() {
		if ( defined( 'CANLIDERS1_VERSION' ) ) {
			$this->version = CANLIDERS1_VERSION;
		} else {
			$this->version = '3.0.0';
		}
		$this->plugin_name = 'canliders1';
	...
	public function check_migration() {
		$previous_version = get_option( 'canliders1_plugin_version' );
		$oldest_version   = get_option( 'bbb_db_version' );
		$new_version      = $this->get_version();
		$migrator         = new Canliders1_Migration( $previous_version, $new_version );

		if ( false === $previous_version && false === $oldest_version ) {
			update_option( 'canliders1_plugin_version', $new_version );
	 ...
	 *
	 * - Canliders1_Loader. Orchestrates the hooks of the plugin.
	 * - Canliders1_I18n. Defines internationalization functionality.
	 * - Canliders1_Admin. Defines all hooks for the admin area.
	 * - Canliders1_Public. Defines all hooks for the public side of the site.
	 *
	...
	private function load_dependencies() {
		 ...
		 * core plugin.
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'includes/class-canliders1-loader.php';

		/**
		 * The class responsible for defining internationalization functionality
		 * of the plugin.
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'includes/class-canliders1-i18n.php';

		/**
		 * The class responsible for defining actions specific to the admin area.
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'admin/class-canliders1-admin.php';

		/**
		 * Registration of necessary components for the plugin.
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'admin/class-canliders1-register-custom-types.php';

		/**
		 * Admin area API
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'admin/class-canliders1-admin-api.php';

		/**
		 * Admin helper.
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'admin/helpers/class-canliders1-admin-helper.php';

		/**
		 * The class responsible for defining all actions that occur in the public-facing
		 * side of the site.
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'public/class-canliders1-public.php';

		/**
		 * The class responsible for migrations from previous versions of the plugin to new ones.
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'includes/class-canliders1-migration.php';

		/**
		 * Public facing plugin API
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'public/class-canliders1-public-room-api.php';
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'public/class-canliders1-public-recording-api.php';

		/**
		 * Shortcodes
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'public/class-canliders1-public-shortcode.php';

		/**
		 * Widget
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'public/class-canliders1-public-widget.php';

		/**
		 * Canliders1 API
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'includes/class-canliders1-api.php';

		/**
		 * Canliders1 Recordings helper
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'public/helpers/class-canliders1-recording-helper.php';

		/**
		 * Canliders1 public view helper
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'public/helpers/class-canliders1-display-helper.php';

		/**
		 * Canliders1 tokens helper
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'public/helpers/class-canliders1-tokens-helper.php';
		...
		/**
		 * Canliders1 permissions helper
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'public/helpers/class-canliders1-permissions-helper.php';

		$this->loader = new Canliders1_Loader();
	 ...
	 *
	 * Uses the Canliders1_I18n class in order to set the domain and to register the hook
	 * with WordPress.
	 *
	 * @since    3.0.0
	 * @access   private
	...
	private function set_locale() {
		...
		$plugin_i18n = new Canliders1_I18n();
	...
	private function define_admin_hooks() {
		...
		$plugin_admin                       = new Canliders1_Admin( $this->get_plugin_name(), $this->get_version() );
		$plugin_admin_api                   = new Canliders1_Admin_Api();
		$plugin_admin_register_custom_types = new Canliders1_Register_Custom_Types();
		...
		// Show update notice.
		$this->loader->add_action( 'in_plugin_update_message-canliders1/canliders1-plugin.php', $plugin_admin, 'canliders1_show_upgrade_notification', 10, 2 );
	...
	/**
	 * Register all of the hooks related to Canliders1_Public the public-facing functionality
	 * of the plugin.
	 *
	 * @since    3.0.0
	 * @access   private
	...
	private function define_public_hooks() {
		...
		$plugin_public               = new Canliders1_Public( $this->get_plugin_name(), $this->get_version() );
		$plugin_public_shortcode     = new Canliders1_Public_Shortcode();
		$plugin_public_room_api      = new Canliders1_Public_Room_Api( $this->get_plugin_name(), $this->get_version() );
		$plugin_public_recording_api = new Canliders1_Public_Recording_Api();
	 ...
	 *
	 * @since     3.0.0
	 * @return    Canliders1_Loader    Orchestrates the hooks of the plugin.
	 */
