...
class Canliders1_Migration {
	...
	private function import_rooms() {
		global $wpdb;
		$old_rooms_table     = 'wp_canliders1';
		$old_room_logs_table = 'wp_canliders1_logs';
		...
		if ( $wpdb->get_var( $old_rooms_query ) === $old_rooms_table ) {
			...
			foreach ( $old_rooms as $old_room ) {
				...
				if ( 0 === $new_room_id ) {
					$this->error_message = sprintf( wp_kses( __( 'Failed to import the room, %s.', 'canliders1' ), array() ), $old_room->meetingNamen );
					return false;
				} else {
			...
			if ( count( $check_old_rooms ) > 0 ) {
				$this->error_message = __( 'Not all rooms were able to be imported to the new version.', 'canliders1' );
				return false;
			} else {
	...
	private function import_permissions() {
		$old_permissions = get_option( 'canliders1_permissions' );
		if ( false === $old_permissions ) {
			delete_option( 'canliders1_permissions' );
			return;
		}
	...
	public function import_from_older_versions() {
		...
		// Set canliders1_plugin_version value.
		update_option( 'canliders1_plugin_version', '1.4.6' );
	...
	public function import_from_older_versions_rooms() {
		...
		$table_name = $wpdb->prefix . 'canliders1';

		// Updates for version 1.3.1 and earlier.
		$canliders1_plugin_version_installed = get_option( 'canliders1_plugin_version' );
		if ( false === $canliders1_plugin_version_installed || ( strcmp( '1.3.1', $canliders1_plugin_version_installed ) <= 0 && get_option( 'bbb_db_version' ) ) ) {

			// Initialize database will create the tables added for version 1.4.6.
			$this->canliders1_init_old_database();
			...
			foreach ( $list_of_meetings as $meeting ) {
				...
				$wpdb->query( $wpdb->prepare( $sql, Canliders1_Admin_Helper::generate_random_code( 6 ), $meeting->meetingID, $meeting->meetingVersion, $meeting->attendeePW, $meeting->moderatorPW ) );
			}
			...
			if ( ! get_option( 'mt_bbb_url' ) ) {
				update_option( 'canliders1_url', 'http://test-install.dogangokce.com/canliders1/' );
			} else {
				update_option( 'canliders1_url', get_option( 'mt_bbb_url' ) );
				delete_option( 'mt_bbb_url' );
			}

			if ( ! get_option( 'mt_salt' ) ) {
				update_option( 'canliders1_salt', '8cd8ef52e8e101574e400365b55e11a6' );
			} else {
				update_option( 'canliders1_salt', get_option( 'mt_salt' ) );
	...
	public function import_from_older_versions_permissions() {
		...
		if ( $canliders1_plugin_version_installed && strcmp( $canliders1_plugin_version_installed, '1.3.3' ) < 0 ) {
			$roles              = $wp_roles->role_names;
			$roles['anonymous'] = 'Anonymous';
			$permissions        = [];
			if ( get_option( 'canliders1_permissions' ) ) {
				$old_permissions = get_option( 'canliders1_permissions' );
				...
				update_option( 'canliders1_permissions', $permissions );
			}
		}
	}
	 ...
	 * @since   1.4.6
	 */
	public function canliders1_init_old_database() {
		...
		// Sets the name of the table.
		$table_name      = $wpdb->prefix . 'canliders1';
		$table_logs_name = $wpdb->prefix . 'canliders1_logs';
        ...
        VALUES ('" . Canliders1_Admin_Helper::generate_random_code( 6 ) . "','Demo meeting', '" . time() . "', 'ap', 'mp');";
		dbDelta( $sql );

		$sql = 'INSERT INTO ' . $table_name . " (meetingID, meetingName, meetingVersion, attendeePW, moderatorPW, recorded)
        VALUES ('" . Canliders1_Admin_Helper::generate_random_code( 6 ) . "','Demo meeting (recorded)', '" . time() . "', 'ap', 'mp', TRUE);";
		dbDelta( $sql );
