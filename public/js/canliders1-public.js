	...
	$( window ).load( function() {
		...
		jQuery( document ).on( 'heartbeat-send', function( event, data ) {
			if ( 0 < $( '#bbb-wait-for-mod-msg' ).length ) {
				data.check_canliders1_meeting_state = true;
				data.canliders1_room_id = $( '#bbb-wait-for-mod-msg' ).data(
					'room-id'
				);
				if ( $( '#bbb-wait-for-mod-msg' ).data( 'temp-room-pass' ) ) {
					data.canliders1_temp_room_pass = $(
						'#bbb-wait-for-mod-msg'
					).data( 'temp-room-pass' );
				}
				if ( $( '#bbb-wait-for-mod-msg' ).data( 'room-username' ) ) {
					data.canliders1_room_username = $(
						'#bbb-wait-for-mod-msg'
					).data( 'room-username' );
				}
			}
		...
		jQuery( document ).on( 'heartbeat-tick', function( event, data ) {
			if ( ! data.canliders1_admin_has_entered ) {
				return;
			}
			window.location.replace( data.canliders1_join_url );
		});
