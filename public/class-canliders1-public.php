...
class Canliders1_Public {
	...
	public function enqueue_styles() {
		...
		wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/canliders1-public.css', array(), $this->version, 'all' );
	...
	public function enqueue_scripts() {
		...
		$translations = array(
			'expand_recordings'   => __( 'Expand recordings', 'canliders1' ),
			'collapse_recordings' => __( 'Collapse recordings', 'bigbluebutotn' ),
			'edit'                => __( 'Edit' ),
			'published'           => __( 'Published' ),
			'unpublished'         => __( 'Unpublished' ),
			'protected'           => __( 'Protected', 'canliders1' ),
			'unprotected'         => __( 'Unprotected', 'canliders1' ),
			'ajax_url'            => admin_url( 'admin-ajax.php' ),
		);

		wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/canliders1-public.js', array( 'jquery' ), $this->version, false );
	...
	public function enqueue_heartbeat() {
		if ( get_query_var( 'canliders1_wait_for_mod' ) ) {
	 ...
	 * @return  Array $vars List of vars that can be queried, including CanliDers1 variables.
	 */
	public function add_query_vars( $vars ) {
		$vars[] = 'canliders1_wait_for_mod';
		return $vars;
	}
	...
	public function bbb_room_content( $content ) {
		...
		$token    = 'z' . $room_id;
		$content .= '[canliders1 token="' . $token . '"]';
		...
		if ( 'true' == $room_can_record ) {
			$content .= '[canliders1 type="recording" token="' . $token . '"]';
		}
	...
	/**
	 * Register canliders1 widget.
	 *
	 * @since   3.0.0
	 */
	public function register_widget() {
		register_widget( 'Canliders1_Public_Widget' );
	}
}
