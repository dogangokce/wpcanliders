 ...
 */
class Canliders1_Public_Room_Api {
	/**
	...
	public function bbb_user_join_room() {
		if ( ! empty( $_POST['action'] ) && 'join_room' == $_POST['action'] && wp_verify_nonce( $_POST['bbb_join_room_meta_nonce'], 'bbb_join_room_meta_nonce' ) ) {
			...
			$access_using_code   = CanliDers1_Permissions_Helper::user_has_bbb_cap( 'join_with_access_code_bbb_room' );
			$access_as_moderator = CanliDers1_Permissions_Helper::user_has_bbb_cap( 'join_as_moderator_bbb_room' );
			$access_as_viewer    = CanliDers1_Permissions_Helper::user_has_bbb_cap( 'join_as_viewer_bbb_room' );
			...
			if ( $access_as_moderator || get_post( $room_id )->post_author == $user->ID ) {
				...
				wp_die( esc_html__( 'You do not have permission to enter the room. Please request permission.', 'canliders1' ) );
			}
	...
	public function get_join_form() {
		...
		if ( array_key_exists( 'room_id', $_POST ) ) {
			$access_using_code   = CanliDers1_Permissions_Helper::user_has_bbb_cap( 'join_with_access_code_bbb_room' );
			$access_as_moderator = ( CanliDers1_Permissions_Helper::user_has_bbb_cap( 'join_as_moderator_bbb_room' ) || ( get_current_user_id() == get_post( $_POST['room_id'] )->post_author ) );
			$access_as_viewer    = CanliDers1_Permissions_Helper::user_has_bbb_cap( 'join_as_viewer_bbb_room' );
	...
	public function bbb_check_meeting_state( $response, $data = [] ) {
		if ( empty( $data['check_canliders1_meeting_state'] ) || empty( $data['canliders1_room_id'] ) ) {
			return $response;
		}

		$username                                    = '';
		$room_id                                     = (int) $data['canliders1_room_id'];
		$entry_code                                  = strval( get_post_meta( $room_id, 'bbb-room-viewer-code', true ) );
		$response['canliders1_admin_has_entered'] = false;

		if ( ! CanliDers1_Permissions_Helper::user_has_bbb_cap( 'join_as_viewer_bbb_room' ) ) {
			$temp_entry_pass = sanitize_text_field( $data['canliders1_temp_room_pass'] );
			if ( ! wp_verify_nonce( $temp_entry_pass, 'canliders1_entry_code_' . $entry_code ) ) {
				$entry_code = '';
			}
		}
		...
		if ( is_user_logged_in() ) {
			...
			$username = sanitize_text_field( $data['canliders1_room_username'] );
		}

		$join_url = Canliders1_Api::get_join_meeting_url( $room_id, $username, $entry_code );

		if ( Canliders1_Api::is_meeting_running( $room_id ) ) {
			$response['canliders1_admin_has_entered'] = true;
			$response['canliders1_join_url']          = $join_url;
		}
	...
	private function join_meeting( $return_url, $room_id, $username, $entry_code, $viewer_code, $wait_for_mod ) {
		$join_url = Canliders1_Api::get_join_meeting_url( $room_id, $username, $entry_code, $return_url );
		...
		if ( $entry_code == $viewer_code && 'true' == $wait_for_mod ) {
			if ( Canliders1_Api::is_meeting_running( $room_id ) ) {
				...
				if ( ! $access_as_viewer ) {
					$query['temp_entry_pass'] = wp_create_nonce( 'canliders1_entry_code_' . $entry_code );
				}
				wp_redirect( add_query_arg( $query, $return_url ) );
			}
		} else {
