 ...
 */
class Canliders1_Tokens_Helper {
	 ...
	 * @since   3.0.0
	 *
	 * @param   Canliders1_Display_Helper $display_helper     Display helper to get HTML from partials.
	...
	public static function join_form_from_tokens_string( $display_helper, $token_string, $author ) {
		...
		if ( count( $rooms ) > 0 ) {
			...
			$content = '<p>' . esc_html__( 'There are no rooms in the selection.', 'canliders1' ) . '</p>';
		}
		return $content;
	}
	...
	public static function recordings_table_from_tokens_string( $display_helper, $token_string, $author ) {
		$manage_recordings               = CanliDers1_Permissions_Helper::user_has_bbb_cap( 'manage_bbb_room_recordings' );
		$view_extended_recording_formats = CanliDers1_Permissions_Helper::user_has_bbb_cap( 'view_extended_bbb_room_recording_formats' );
		$tokens_arr                      = preg_split( '/\,/', $token_string );
		$room_ids                        = array();
		$content                         = '';
	...
	public static function find_room_id_by_token( $token, $author ) {
		...
		if ( ! user_can( $author, 'edit_bbb_rooms' ) ) {
			self::$error_message = esc_html__( 'This user does not have permission to display any rooms in a shortcode or widget.', 'canliders1' );
			return 0;
		}
	...
	private static function check_if_room_exists_for_new_token_format( $token, $author ) {
		...
		if ( false !== $room && null !== $room && 'bbb-room' == $room->post_type ) {
			if ( 'publish' != $room->post_status ) {
				self::set_error_message( sprintf( wp_kses( __( 'The token: %s is not associated with a published room.', 'canliders1' ), array() ), $token ), $author );
				return 0;
			}
			return $room->ID;
		} else {
	...
	private static function check_if_room_exists_for_old_token_format( $token, $author ) {
		...
		self::set_error_message( sprintf( wp_kses( __( 'The token: %s is not associated with an existing room.', 'canliders1' ), array() ), $token ), $author );
		return 0;
	}
	...
	private static function get_recordings( $room_ids ) {
		$recording_helper = new Canliders1_Recording_Helper();
	...
	private static function set_error_message( $detailed_message, $author ) {
		if ( current_user_can( 'edit_others_bbb_rooms' ) || get_current_user_id() == $author ) {
			...
			self::$error_message = esc_html__( 'The room linked to this resource is not configured correctly.', 'canliders1' );
		}
	}
}
