 ...
 */
class Canliders1_Recording_Helper {
	...
	public function get_recordings_based_on_capability( $room_ids ) {
		...
		if ( $manage_recordings ) {
			$this->recordings = Canliders1_Api::get_recordings( $room_ids, 'published,unpublished' );
		} else {
			$this->recordings = Canliders1_Api::get_recordings( $room_ids, 'published' );
		}
		return $this->recordings;
	}
	...
	private function filter_managed_recording( $recording ) {
		if ( 'true' == $recording->protected ) {
			...
			$recording->protected_icon_title   = __( 'Protected', 'canliders1' );
			...
			$recording->protected_icon_title   = __( 'Unprotected', 'canliders1' );
		}
