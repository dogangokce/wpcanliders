 ...
 */
class Canliders1_Public_Recording_Api {
	...
	public function set_bbb_recording_publish_state() {
		...
		if ( CanliDers1_Permissions_Helper::user_has_bbb_cap( 'manage_bbb_room_recordings' ) ) {
			if ( array_key_exists( 'meta_nonce', $_POST ) && array_key_exists( 'record_id', $_POST ) &&
				...
				$return_code = Canliders1_Api::set_recording_publish_state( $record_id, $value );
	...
	public function set_bbb_recording_protect_state() {
		...
		if ( CanliDers1_Permissions_Helper::user_has_bbb_cap( 'manage_bbb_room_recordings' ) ) {
			if ( array_key_exists( 'meta_nonce', $_POST ) && array_key_exists( 'record_id', $_POST ) &&
				...
				$record_id   = sanitize_text_field( $_POST['record_id'] );
				$value       = sanitize_text_field( $_POST['value'] );
				$return_code = Canliders1_Api::set_recording_protect_state( $record_id, $value );
	...
	public function trash_bbb_recording() {
		...
		if ( CanliDers1_Permissions_Helper::user_has_bbb_cap( 'manage_bbb_room_recordings' ) ) {
			if ( array_key_exists( 'meta_nonce', $_POST ) && array_key_exists( 'record_id', $_POST ) &&
				...
				$record_id   = sanitize_text_field( $_POST['record_id'] );
				$return_code = Canliders1_Api::delete_recording( $record_id );
	...
	/**
	 * Send recording metadata to Canliders1 API.
	 *
	 * @since   3.0.0
	 *
	...
	public function set_bbb_recording_edits() {
		...
		if ( CanliDers1_Permissions_Helper::user_has_bbb_cap( 'manage_bbb_room_recordings' ) ) {
			if ( array_key_exists( 'meta_nonce', $_POST ) &&
				...
				$return_code = Canliders1_Api::set_recording_edits( $record_id, $type, $value );
